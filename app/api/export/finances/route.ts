...
export async function GET(req: NextRequest) {
  ...
  if (type === 'payments') {
    const [{ data }, { data: teams }] = await Promise.all([
      db.from('payments')
        .select('amount_cents, tax_cents, status, payment_method, payment_type, paid_at, created_at, notes, league_id, team_id, registration_id, payer:profiles!payments_user_id_fkey(full_name, email), registration:registrations!payments_registration_id_fkey(guest_name)')
        .eq('organization_id', org.id).in('status', ['paid', 'manual']),
      db.from('teams').select('id, name').eq('organization_id', org.id),
    ])
    const teamNameById = new Map((teams ?? []).map((t) => [t.id, t.name]))
    const seenTeams = new Set<string>()
    columns = ['date', 'event', 'payer', 'type', 'method', 'status', 'amount', 'tax_included', 'counted_in_report', 'notes']
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    rows = ((data ?? []) as any[])
      .filter((p) => inDateRange(p.paid_at, p.created_at))
      .map((p) => {
