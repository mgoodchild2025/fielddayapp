...
export async function POST(req: NextRequest) {
  if (checkoutRateLimiter.check(getClientIp(req)).limited) {
  ...
  if (installment.stripe_checkout_session_id) {
    ...
    if (orgSettings.data?.stripe_secret_key) {
      const orgStripe = new Stripe(orgSettings.data.stripe_secret_key, { apiVersion: '2026-05-27.dahlia' as const })
      try {
