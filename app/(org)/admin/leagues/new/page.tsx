...
export default function NewLeaguePage() {
  ...
  const { register, handleSubmit, watch, setValue, formState: { errors } } = useForm<FormData>({
    ...
    defaultValues: { league_type: 'team', sport: 'beach_volleyball', payment_mode: 'per_player', min_team_size: 4, max_team_size: 8, price_cents: 0 },
  })
  ...
  async function onSubmit(data: FormData) {
    ...
    if (result.error) {
      setError(result.error === 'UPGRADE_REQUIRED' ? 'Your plan only allows 1 active league. Upgrade to create more.' : result.error)
      setLoading(false)
    } else {
