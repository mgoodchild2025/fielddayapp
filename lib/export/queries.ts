...
export async function fetchLeagues(db: DB, orgId: string) {
  const { data } = await db
    .from('leagues')
    .select('id, name, sport, description, status, event_type, created_at, updated_at, slug, max_teams, max_participants, registration_opens_at, registration_closes_at, start_date, end_date')
