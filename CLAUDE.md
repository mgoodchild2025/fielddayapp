...
The four endpoints under `app/api/cron/*` (`reminders`, `cleanup-exports`, `data-retention`, `hibernate-resume`) are GET routes guarded by `Authorization: Bearer $CRON_SECRET`. Railway schedules a **service**, not a URL, so cron runs through `scripts/run-cron.mjs` (dependency-free; takes job names as arguments, calls each endpoint in sequence, retries once, exits non-zero if any fail).
- **No config-as-code.** Railway deprecated it and services created after 2026-08-28 cannot opt in, so every cron setting is a dashboard setting. Select the small image with the service variable `RAILWAY_DOCKERFILE_PATH=Dockerfile.cron`; set the start command, the UTC schedule (≥5 min apart), `CRON_SECRET`, and restart policy **Never** in the service's settings. Only the web service still reads `railway.toml`.
