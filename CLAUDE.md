...
- **Reminders (cron) are push-first.** The reminders cron creates bell rows via `createNotifications` for the night-before digest (`game_reminder`), game-day morning (`game_day`), org-configured pre-game minutes (`game_reminder`), and session reminders (`session_reminder`); rows carry `data.until` (game/session start) and step 0 of the cron marks stale ones read. Channel routing is `lib/reminder-channels.ts#decideReminderChannels` (pure, tested): push unless `profiles.push_reminders_enabled` is off; SMS by the existing gates EXCEPT a player with a push subscription for that org is not texted unless `profiles.sms_also_when_push` (migration 192). `sms_game_day_enabled` gates the game-day kind on both channels. Email is untouched. Admin → Settings → Notifications shows how many members are reachable by push.
- **Offline & share (phase 5).** The root worker (`app/sw.js/route.ts`) caches ONLY `/dashboard`, `/schedule`, `/standings` (network-first, last good copy offline; never caches redirects; drops the cache on a redirected navigation and on the `clear-cache` message the logout forms send via `clearOfflineCache()`), plus `/_next/static` cache-first. Every other navigation is network-or-offline-page — never add more pages without thinking about shared devices. Manifest carries `shortcuts` and, on org hosts, a `share_target` posting to `/share/inbox`: the worker intercepts the POST, parks files in the `fieldday-share-inbox` cache and redirects to `/share` (`components/pwa/share-inbox.tsx` → signed Cloudinary upload via `lib/cloudinary-upload-client.ts` → `recordEventMediaUpload`, pending approval as usual). `app/share/inbox/route.ts` is the no-worker fallback (files can't be kept; redirects with a hint).
- **Install metrics (phase 6).** `pwa_launch_logs` (migration 193; PK org+user+day, `standalone`, `platform`, `launches`) is upserted once per browser session by `PwaRegistrar` → `actions/pwa.ts#logPwaLaunch` (a standalone launch wins the day). `/super/phone-alerts` reads it with `push_subscriptions` (reach, `failed_at` in 7d) and `org_members` for per-org install rate, push reach, and platform split.

## Exhibition games
`games.is_exhibition` (migration 194, default false): played, scored, live-scored and stat-tracked like any other game, but **never moves the table**. `lib/standings.ts#countsForStandings(game)` is the one rule — every standings fold (event page, admin standings + pools, dashboard, team stats, TV display data, career W/L in `lib/career.ts`) gates on it, so add it to any new fold and select `is_exhibition` alongside. Set via the Add Game / Edit Game forms (`addGame`/`updateGame` accept `isExhibition`; `updateGame` only touches the column when the caller sends it). Badged "Exhibition" (violet) on the admin schedule table, the public `/schedule`, and event-page schedule rows. Player stats entered for exhibition games still count — they are personal records, not league position.

## Scheduled jobs (Railway cron)
The four endpoints under `app/api/cron/*` (`reminders`, `cleanup-exports`, `data-retention`, `hibernate-resume`) are GET routes guarded by `Authorization: Bearer $CRON_SECRET`. Railway schedules a **service**, not a URL, so cron runs through `scripts/run-cron.mjs` (dependency-free; takes job names as arguments, calls each endpoint in sequence, retries once, exits non-zero if any fail). Built from `Dockerfile.cron` — deliberately not the app image, since these services only make an HTTP call to the already-deployed web service. `railway.cron.toml` is their config-as-code file (no healthcheck, `restartPolicyType = "NEVER"`); the web service keeps `railway.toml`.
...
- One schedule per Railway service: a frequent service runs `reminders`, a daily one runs the other three in sequence. Schedules are UTC, minimum 5 minutes apart.
