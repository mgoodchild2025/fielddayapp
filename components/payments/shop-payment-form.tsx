...
export function ShopPaymentForm({ mode, instructions }: Props) {
  ...
  function handleSave() {
    ...
    startTransition(async () => {
      const result = await saveShopPaymentSettings({
        shopPaymentMode: selectedMode,
        manualPaymentInstructions: selectedMode === 'manual' ? text : null,
      })
      if (result.error) {
    ...
    <div className="space-y-4">
      ...
      {selectedMode === 'manual' && (
        <div>
          ...
          <textarea
            ...
            rows={4}
            placeholder="e.g. Send e-transfer to treasurer@myclub.ca with your name and order number as the message. Cash accepted at any game night."
          ...
          <p className="text-xs text-gray-400 mt-1">
            Shown to players after they place a manual order.
          </p>
        </div>
      )}
      ...
      {selectedMode === 'stripe' && (
        <p className="text-xs text-gray-500 bg-gray-50 rounded-lg px-3 py-2.5 border">
          ...
          shop orders will fall back to manual payment automatically.
        </p>
      )}
