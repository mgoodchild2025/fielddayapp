...
export function EventCalendarSubscribeButton({ slug, calendarToken, host }: Props) {
  const [open, setOpen] = useState(false)

  const feedPath = `/api/events/${slug}/calendar.ics?token=${calendarToken}`
  ...
  if (!open) {
    return (
      <button
        ...
        className="mt-3 w-full py-2 rounded-md text-sm font-medium border hover:bg-gray-50 transition-colors text-gray-600 flex items-center justify-center gap-2 bg-white"
      >
  ...
  return (
    <div className="mt-3 border rounded-md p-4 bg-gray-50">
      ...
      <p className="text-xs text-gray-500 mb-3">
        Your calendar will stay in sync as sessions are added, moved, or cancelled.
      </p>
